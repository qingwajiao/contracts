...
contract LPMining is Ownable {
     ...
     */
    function add(uint256 _allocPoint, uint256 _duration, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            arc.safeTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accARCPerShare).div(1e12);
