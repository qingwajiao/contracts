...
contract CakePool is Ownable, Pausable {
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                // Calculate the user's current token amount and update related parameters. 计算用户当前的令牌数量并更新相关参数
    ...
    function depositOperation(
        ...
        // Harvest tokens from Masterchef. 从主厨那里收获代币
        harvest();
        ...
        // Calculate the boost weight share. 计算提升重量份额
        if (user.lockEndTime > user.lockStartTime) {
            // Calculate boost share.提升份额 = 用户质押解到期 - 用户质押开始时间       *  提升权重     / 持续时间因子
